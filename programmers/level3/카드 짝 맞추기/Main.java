...
//        System.out.println(orders);
    ...
    public static int search(Cursor cursor, int target){
        Queue<Pos> q = new LinkedList<>();
        boolean[][] visited = new boolean[4][4];
        q.offer(new Pos(cursor.x, cursor.y, 0));
    ...
    public static Pos check(int x, int y, int dir){
        ...
        while(x >= 0 && x < 4 && y >= 0 && y < 4){
            if(copyBoard[x][y] != 0) return new Pos(x, y, 0);

            x += dx[dir];
            y += dy[dir];
        }

        return new Pos(x - dx[dir], y - dy[dir], 0);
    }
}
